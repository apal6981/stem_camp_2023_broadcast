...
static volatile size_t message_len;
static song_note **notes = NULL;
static song_meta_data meta_data = {0, 0xFFFF};
static volatile uint8_t packet_arrival = 0;
...
static uint16_t num_populated = 0;
...
// delay(1000);
  ...
  if (packet_arrival)
  {
    packet_arrival = 0;
    cur_time = millis();
    uint16_t *_16bytes = (uint16_t *)message_buffer;
    // Serial.printf("%d %d  %d \t%d ", ntohs(_16bytes[0]), ntohs(_16bytes[1]), ntohs(_16bytes[2]), read);
    if (!check_checksum(_16bytes, message_len / 2 - 1))
    {
#ifdef COM
      Serial.println("Checksum failed");
#endif
    ...
    // Serial.printf("%d %d %d + %d = %d, %d", num_bytes, num_bytes / 2 - 1, ntohs(_16bytes[0]), checksum_calc(_16bytes + 1, num_bytes / 2 - 1), ntohs(_16bytes[0]) + checksum_calc(_16bytes + 1, num_bytes / 2 - 1), check_checksum(_16bytes, num_bytes / 2 - 1));
    header p_head;
    get_header(p_head, (uint8_t)message_buffer[2]);
    // Serial.printf("%d %d %d    ", p_head.p_type, p_head.num_packets, p_head.repeat);
    ...
    case 0: // Announcment of song types and how many notes
    {
      ...
      uint8_t number_of_parts = (uint8_t)message_buffer[3];
#ifdef COM
...
void play_note_color(uint16_t index, uint8_t on)
{
  ...
  if (on)
  {
    // ledcSetup(0, notes[song_index]->note - PITCH_OFFSET, 8);
    // ledcAttachPin(TONE_PIN, 0);
    ledcWriteTone(0, note_pitches[notes[song_index]->note - PITCH_OFFSET]);
    // analogWriteFrequency(TONE_PIN, notes[song_index]->note - PITCH_OFFSET);
